...
class Stack(list):
    ...
    def sort_self(self, seq):
        for i in range(len(seq)):
            if type(seq[i]) == str:
                seq[i] = custom_types.Str(seq[i])
            if type(seq[i]) == int:
                seq[i] = custom_types.Int(seq[i])
            if type(seq[i]) == list:
                seq[i] = custom_types.List(seq[i])
            if type(seq[i]) == bool:
                seq[i] = custom_types.Bool(seq[i])
            if type(seq[i]) == float:
                seq[i] = custom_types.Float(seq[i])
...
COMMANDS = {
    ...
    'Q':lambda i,s: s.push(''.join(set_(s.pop(i)))),
    ...
    'Y':lambda i,s: s.push('\n'.join(s.pop(i))),
    'Z':lambda i,s: s.push(''.join(map(''.join, zip(s.pop(i), s.pop())))),
    ...
    'h':lambda i,s: s.push(regex.flatten(s.pop(i))),
